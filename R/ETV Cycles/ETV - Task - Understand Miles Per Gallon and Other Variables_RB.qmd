...
To bring data from an outside source into your working environment
...
Where you have columns and rows that denote variables and observations. Variables are the columns and rows are the observations
...
numeric (quantitative), character (categorical), logical (boolean)
...
A data frame is a table or two dimensional array that is structured in a way that each column contains values of one variable and each row contains one set of values from each column
...
Data transformation is the process of transforming data from one format or structure into another format or structure
...
The filter function is used to filter columns from a data set and remove anything that doesn't fit the criteria
...
Mutate is used to create a new variable or modify an existing variable in the data frame
...
Is a graphical representation of data that is used to communicate information clearly and efficiently through graphical means
...
p1 = mtcars %>% 
  select(mpg) %>% 
  ggplot(aes(x=mpg)) + geom_density() +
  labs(
    x = "Miles Per Gallon (mpg)",
    title = "Denisty Plot of mpg"
  )
```
