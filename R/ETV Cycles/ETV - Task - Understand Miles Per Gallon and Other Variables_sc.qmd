...
```{r}
library(dplyr) # data transfomation 
...
To bring Data from a source into your working environment
...
Each Variable is stored in its own column and each observation is stored in its own row.
...
Character (categorical, catagorical binary)
...
A data frame is a table or two dimensional array like structure in which each column contains values of one variable and each row contains one set of values of each column.
...
the process of converting data from structure or formateinto another format or structure
...
the filter() function selects rows based on a critieria (logical condition)
...
the mutate() function is used to create a new variable or modify an existing variable
...
Data visualizations are the graphical representation of data, it is used to clearly and efficiently communicate data through graphical means.
...
subtask3 <- mtcars %>% 
  ...
  labs(title = "Scatter Plot of MPG and HP" ,  x = "Horse Power",  y = "Miles per Gallon")+
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5)) +
