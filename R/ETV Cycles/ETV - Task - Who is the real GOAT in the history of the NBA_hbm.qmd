...
```{r}
## Code here
path_location_to_nba_file <- here("Data_1", "nba_goats.xlsx")
```
...
'bind_rows()' combines data frames by stacking them vertically.

Very useful when they have the same variable names (fields)
...
Combines data frames by adding columns side by side. It is useful when we have the same number of rows in each data frame
