...
mj_df <- read_xlsx(path_location_to_nba_file, sheet = 3) ## Provide the number for MJ
```
...
variable_names_df <- read_xlsx(path_location_to_nba_file, sheet = "variable_names")
...
str_detect() function is used to detect specific sub strings in a character column

**Q20.** What is the purpose of `case_when()`?

case_when() function is used to make new categorical variables based on a condition.

**Q21.** What other function could we have used instead of `case_when`? Hint: Think of HW 1.

```{r}
mod_4_df <- mod_2_nba_goats_df %>% 
  mutate(
    Outcome_Renamed = if_else(
...
Gives the same output as the above chunk.

Means everything else –\> (\>30) gets assigned "High"
...
-   **Task Description**: Create a combined boxplot and violin plot comparing players against their points scored (`PTS`) during their first season, including mean markers and axis labels (make sure the axis titles are bolded). Interpret the distribution of points scored by each player and identify which player had the highest variability in points.
-   group_by Name
-   Find out where the first season stops
    -   calculate mean, sd, number of games played, total points ...
-   Plot based on the previous group_by summary
-   Figure out how to add a box plot and a violin plot on the same output
-   Do Axis titles (bolded)
...
axis.text refers to KB, LJ, and MJ while axis.title refers to Player Name.
...
```{r}
## Student Version
kb_summary_statistics_df <- nba_goats_df %>% 
  filter(str_detect(Name, "KB"), Season %in% str_c("season_", 1:7)) %>% 
  group_by(Season) %>% 
  summarise(`Mean KB PTS` = mean(PTS, na.rm = TRUE), `SD` = sd(PTS, na.rm = TRUE), `Number of Games` = n()) %>% 
  arrange(desc(`SD`))   # could also use -`SD`
```
...
ci_kb_df <- kb_summary_statistics_df %>% 
  mutate(se = `SD`/sqrt(`Number of Games`),
...
```{r}
## Student Version
first_season_df <- nba_goats_df %>%
  filter(Season == "season_1") %>% 
  mutate(new_PTS = case_when(
    PTS < 20 ~ "Low",
    PTS >= 20 & PTS <= 30 ~ "Medium",
    PTS > 30 ~ "High"
  )) %>% 
  filter(!is.na(new_PTS))
...
first_season_df %>% 
  select(Name, new_PTS) %>% 
  ggplot(aes(x = Name, fill = new_PTS)) +
  geom_bar(position = "dodge")
```

"fill" fills the frequency of the occurrences
...
```{r}
# Bind columns of a and b
df_ab <- bind_cols(df_a, df_b)

# Bind rows of b and c
df_bc <- bind_rows(df_b, df_c)

df_all <- bind_rows(df_a, df_b, df_c)
```

```{r}
ggplot(data = nba_goats_df, aes(x = MP, y = PTS)) + 
  geom_point() + 
  theme()
```
