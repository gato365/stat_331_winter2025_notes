...
```{r}
## Enter Libraries
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
...
>>>>>>>> main:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2 _ijw.qmd
```
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
Creates a new data frame that contains rows with matching column names (variables or IDs) in both data frames.
========
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
You can see that the rows move to the resulting data frame only when there is a match.
========
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
Uses the first data frame as a base (called the left data frame), keeping all the rows, then it joins based on a common column (variable) and adds a new column (brought over from the right data frame).
========
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
You can see that all the rows from the left data frame remain in the resulting table regardless if there is a match. When there is a match the values from the right are added to the left, if there is no match then the column from the data frame are filled with `NA`.
========
...
identical_data <- all.equal(left_join_df_sorted, right_join_df_sorted)
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
In a semi join only the rows from the left data frame that have a match in the right data frame are included in the resulting table. Unlike other joins, the semi join does not include any columns from the right data frame.
========
...
pivot_longer(data = wide_df, cols = c(Math, Science), names_to = "Subject", values_to = "Score")
========
```{r}
wide_df
```
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
To transform a wide dataframe at which the selected variables are the key and their specified elements are the values.
========
...
Values are the selected column elements.
========
Keys are the selected columns names
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
Transform a long formatted data frame so that each key has a column to denote the variable and its values.
========
...
<<<<<<<< HEAD:R/ETV Cycles/ETV - Task - Who is the real GOAT of all time in the NBA Part 2_ac.qmd
# Find total points of each division and stage
# Find the max() for each stage
# Join (semi-join) each of these data frames in which max is found in total points
========
