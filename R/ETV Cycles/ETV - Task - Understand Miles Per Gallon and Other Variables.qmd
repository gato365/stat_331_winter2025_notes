...
Table/2D array where cols are variables and rows contain a set of one of each column.
...
```{r}
select_mpg_mtcars_df <- mtcars |> select(mpg)
```
...
The select function is used to select columns from a dataframe
...
It removes observations that do not fit the criteria
