...
-   in this class we are doing 55-60% data transformation
...
-   could of also done select mtcars (mpg) but this is better

-   \_df for it being a dataframe -- also name is descriptive (not true on homework though)

-   can do mpg, cyl to include both, or -mpg to remove it -- lot of versatility here

`<-` is the assignment (STORE) operator. It is used to assign the output of a function to a variable.
...
-   Number of rows change, columns stay the same (unlike select) -- can filter in view and see none are under 2 now

    The filter function considers rows based on a criteria (logical condition)

-   In homework exercise with multiple conditions (I assume you use a comma)
...
-   If you say, new_var=1 each row will have the value for 1 for that new variable
