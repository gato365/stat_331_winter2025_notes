...
```{r}
str(mtcars)
## Provides all variables and their data types
```
...
If you did select(-mpg), it would select everything BUT mpg.
...
The filter function filters out certain variables based on a given criteria
...
The mutate function creates a new column which can be referenced to an existing variable
...
Data visualizations are a graphical representation of data. It is used to communicate information clearly and efficiently through graphical means
...
mtcars %>% 
  ...
  labs(title = "Scatter Plot of MPG on HP", x= "Miles Per Gallon", y= "Horsepower")+
  theme_bw()+
  theme(plot.title=element_text(hjust = 0.5))
```
...
```{r}
mutate_wt_lbs_mtcars_df <- mtcars %>% 
  mutate(wt_lbs = wt*2000)
```

```{r}
mutate_wt_lbs_mtcars_df %>% 
  filter(cyl==6) %>% 
  select(wt_lbs, mpg) %>% 
  ggplot(aes(x=wt_lbs, y=mpg))+
  geom_point(color="blue", size=3)+
  labs(title = "Scatter Plot of MPG on Weight", 
       x= "Weight in Lbs", y= "MPG")+
  geom_smooth(method="lm", se=FALSE)+
  theme_bw()+
  theme(plot.title=element_text(hjust = 0.5))
