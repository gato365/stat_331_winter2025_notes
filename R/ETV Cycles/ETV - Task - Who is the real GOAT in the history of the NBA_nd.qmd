...
Some things we could analyze based on the provided variables are points per game
...
# Doesn't work (only gets the sheet for Kobe Bryant)
```
...
mj_df <- read_xlsx(path_location_to_nba_file, sheet = "MJ") ## Provide the name for MJ's sheet
```
...
The `bind_rows` function allows you to combine data frames by stacking them vertically. It is very useful when multiple data frames have the same columns.

**Q11.** What happens if the column names do not match when using `bind_rows()`? For example change `score` to `score_ex` in `ex_1_df_b`.

Any columns that occur in 1 data frame but not the other will still show up, but there will be missing values, which show up as `NA`.
...
`bind_cols` combines data frames by adding columns side by side. It is useful when we have the same number of rows in each data frame.

**Q13.** What happens if the number of rows do not match when using `bind_cols()`?

```{r}
ex_1_combined_cols <- bind_cols(ex_1_df_a, ex_1_combined_rows)
```

**Q14.** What could be dangerous with `bind_cols` when working with real data?

`bind_cols` only works well if the rows are aligned with each other, which is important with real data. A join can be used instead of using `bind_cols`.
...
`bind_rows`, since each data frame has the same columns, but the rows (which represent games) should be combined by binding them together.

**Q16.** Go ahead and bind the data frames together.

```{r}
nba_goats_df <- bind_rows(kb_df, lj_df, mj_df)
```
...
Variables like season, game location, game outcome, and team should be factors. This is because these variables have a limited set of possible values that can be used for categorization.
...
mod_2_nba_goats_df <- nba_goats_df %>%
  mutate(Name_Factor = as_factor(Name)) %>%
  mutate(Name_Factor = fct_relevel(Name_Factor, c("LJ", "MJ", "KB")))
```
...
The `str_detect()` function is used to detect specific substrings in a character column.

**Q20.** What is the purpose of `case_when()`?

The `case_when()` function is used to make new categorical variables based on a condition.
...
```{r}
mod_6_nba_goats_df <- mod_3_nba_goats_df %>%
  filter(Name == "MJ") %>%
  mutate(
    PTS_Category = case_when(
      PTS < 20 ~ "Low",
...
kb_summary_stats_df <- nba_goats_df %>%
  ...
  group_by(Season) %>%
  summarise(`Mean KB Points` = mean(PTS, na.rm = TRUE), `SD KB Points` = sd(PTS, na.rm = TRUE), `Number of Games` = n()) %>%
  arrange(desc(`SD KB Points`))
```
...
ci_kb_df <- kb_summary_stats_df %>%
  mutate(se = `SD KB Points` / sqrt(`Number of Games`),
         lower_bound = `Mean KB Points` - 1.96 * se,
         upper_bound = `Mean KB Points` + 1.96 * se
  )
```
...
## Student Version
ci_kb_df %>%
  select(lower_bound, upper_bound, `Mean KB Points`, Season) %>%
  ggplot(aes(x = Season, y = `Mean KB Points`)) +
  geom_point() +
...
nba_goats_df %>%
  ...
  geom_point() +
  scale_color_manual(values = c("KB" = "purple", "LJ" = "gold", "MJ" = "red")) +
  facet_grid(Season ~ Name) +
  labs(x = "Minutes Played", y = "Points", title = "Points by Minutes Played", subtitle = "According to Season and Player") +
  theme_bw() +
...
nba_goats_df %>%
  filter(Season == "season_1") %>%
  mutate(PTS_Category = case_when(
    PTS < 20 ~ "Low",
    PTS >= 20 & PTS < 30 ~ "Medium",
    PTS >= 30 ~ "High"
  )) %>%
  mutate(PTS_Category_Factor = as_factor(PTS_Category)) %>%
  mutate(PTS_Category_Factor = fct_relevel(PTS_Category,
                                           c("Low", "Medium", "High"))) %>%
  filter(!is.na(PTS_Category_Factor)) %>%
  ggplot(aes(x = Name, fill = PTS_Category)) +
