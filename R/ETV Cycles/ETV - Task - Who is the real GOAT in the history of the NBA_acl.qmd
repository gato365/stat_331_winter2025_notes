...
MP (minutes played), FG_Percent, 3P_Percent, FT_Percent, PTS
...
install.packages("here")   ## Locate files
...
library(dplyr)   ## Data Transfomations
...
#path_location_to_nba_file <- "C:/Users/arian/OneDrive/Desktop/stat_331_winter2025/stat_331_winter2025_notes/Data"
```
...
**Q7.** Get KB and the variable names in anyway you would like.

```{r}
kb_df <- read_xlsx(path_location_to_nba_file, sheet = 'KB') ## Provide the number for KB
...
"bind_rows()" combines data frames by stacking them vertically, it is very useful when multiple data frames have the same columns

**Q11.** What happens if the column names do not match when using `bind_rows()`? For example change `score` to `score_ex` in `ex_1_df_b`.

It will add the rows to the new df
...
bind_rows combines data frames by adding columns side-by-side. It is useful when we have the same number of rows in each data frame

**Q13.** What happens if the number of rows do not match when using `bind_cols()`?

Error due to size, it won't work

**Q14.** What could be dangerous with `bind_cols` when working with real data?

Data could get mismatched, you want to use a join instead
...
The str_detect() function is used to detect certain substrings in a character column
...
mod_4_nba_goats_df <- mod_2_nba_goats_df %>% 
  mutate(
    Game_Outcome_Renamed = if_else(str_detect(Game_Outcome,"W"), "Win","Loss"))
```
...
mod_5_nba_goats_df <- mod_3_nba_goats_df %>%
  ...
  mutate(
    PTS_Category = case_when(
      PTS < 20 ~ "Low",                  # Condition 1
      PTS >= 20 & PTS <= 30 ~ "Medium",  # Condition 2
      TRUE ~ "High"                      # Condition 3
    )
  )
...
Means (highest to lowest): MJ, LJ, KB
...
```{r}
## Student Version
kb_summary_statistics_df <- 
  mod_4_nba_goats_df %>%
  filter(str_detect(Name, "KB"), Season %in% str_c("season_",1:7)) %>% 
  group_by(Season) %>% 
  summarise(`Mean KB Points` = mean(PTS, na.rm = TRUE), `SD KB Points` = sd(PTS, na.rm = TRUE), `# of Games` = n()) %>% 
    arrange(-`SD KB Points`) ## or desc() in place of -
```
...
```{r}
## Student Version
ci_kb_df <- kb_summary_statistics_df %>% 
  mutate(se = `SD KB Points`/sqrt(`# of Games`),
...
nba_goats_df %>% 
  ...
  # mutate(Season = str_replace("season_","_"," ")) %>% 
  ggplot(aes(x = MP, y = PTS, color = Name)) +
  geom_point() +
  scale_color_manual(values = c("KB" = "purple","LJ" = "gold","MJ" ="red")) +
  facet_grid(Season ~ Name) +
  labs(x = "Minutes Played", y = "Points", title = "Points by Minutes Played", subtitles = "According to Season and Player") +
  theme_bw() +
...
```{r}
## Student Version
first_season_nba_goats_df <- nba_goats_df %>% 
  filter(Season == "season_1") %>% 
  mutate(PTS_Category = case_when(
      PTS < 20 ~ "Low",                  # Condition 1
      PTS >= 20 & PTS <= 30 ~ "Medium",  # Condition 2
      TRUE ~ "High")) %>%                # Condition 3
  filter(!is.na(PTS_Category))


first_season_nba_goats_df %>% 
  select(Name,PTS_Category) %>% 
...
```{r}
ggplot(data = mod_5_nba_df, aes(x = MP, y = PTS)) +
  geom_point() +
  theme_bw()
```
