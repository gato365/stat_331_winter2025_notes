...
library(ggplot2)    ## Data Visulizations
...
\`bind_row\` combines data frames by stacking them vertically, it is very useful when muhltiple data frames have same columns
...
\`bind_rows\` combines data frames by adding columns side by side, it useful when we have the same number of rows in each data frame
...
mod_3_nba_goats_df <- nba_goats_df %>%
  mutate(
...
```{r}
mod_4_nba_goats_df <- nba_goats_df %>% 
  mutate(Game_Outcome_renamed = if_else(str_detect(Game_Outcome,"W"), "Win","Loss"))
```
...
mod_5_nba_goats_df <- nba_goats_df %>%
...
```{r}
mod_6_nba_goats_df <- nba_goats_df %>%
  filter( Name == "MJ") %>%  # Focus on Michael Jordan's data
  mutate(
    PTS_Category = case_when(
      FALSE ~ "High",
...
```{r}
## Student Version
kb_summary_stat_df <- nba_goats_df %>% 
  filter(str_detect(Name,"KB"),Season %in% str_c("season_",1:7)) %>% 
  group_by(Season) %>% 
  summarise(`Mean KB PTS` = mean(PTS,na.rm = TRUE), `SD KB PTS`= sd(PTS,na.rm = TRUE), `Number of Games` = n()) %>% 
...
nba_goats_df %>% 
  ...
  labs(x = "Minutes PLaye", y = "Points", title = "Points by Minutes Played", subtitle = "According to Season and Player") +
  theme_bw() +
