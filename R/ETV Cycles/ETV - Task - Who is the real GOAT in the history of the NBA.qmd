...
```{r}
ath_location_to_nba_file
```

------------------------------------------------------------------------

**Q5.** What number should be place in sheet to navigate to LeBron's Sheet?
LeBron's sheet in the excel file is the 2nd sheet.
...
```{r}
kobe_df <- read_xlsx(path_location_to_nba_file, sheet = 1) ## Provide the number for KB
```
