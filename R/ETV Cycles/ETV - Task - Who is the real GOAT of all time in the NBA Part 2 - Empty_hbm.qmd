...
```{r}
## Specify Path
nba_goats_path <- here("Data", "nba_goats.xlsx")
```

```{r}
## Read in Sheets
mj_df <- read_xlsx(nba_goats_path, sheet = "MJ")
div_df <- read_xlsx(nba_goats_path, sheet = "divisions")
```
...
If there's not a match, they are dropped from the resulting data frame.
...
The rows move to the resulting table only when there is an exact match in both data frames based on the shared Key.
...
Keeps all the rows from the left data frame (first), and matches from the right data frame (second).
...
You can see that all the rows from the left data frame remain in the resulting table regardless of if there is a match in the right data frame. When there is a match, the values from the right are added to the left. If there is not match, the column from the right data frame are filled with NA.
...
It keeps all rows from both data frames
...
All rows from both data frames are including in the resulting table, regardless of if there is a match
...
Keeps rows from names_df that had a match in scores_df but does not keep columns from scores_df. It is a filtering function.
...
In a semi join, only the rows from the 1st data frame (on the left) that have a match in the second data frame (right) are included in the resulting table. Unlike other join, the semi join does not include any columns from the second data frame.
...
Joins where there is not a match across data frames –\> opposite of semi_join

In an anti_join, the resulting table includes only rows from the 1st data frames on the left that do not match in the 2nd data frame. This join filters out any row in the 1st data frame that share a key with the second data frame.
...
```{r}
## Join The Data Frame
left_j_mj <- left_join(mj_df, div_df, by = c("Opp" = "Abbreviation"))

inner_j_mj <- inner_join(mj_df, div_df, by = c("Opp" = "Abbreviation"))
...
To transform a wide data frame at which the selected variables are the key and their specified elements are the values
...
pivot_wider(data = long_df, names_from = "Subject", values_from = "Score")
```
...
Is to transform a long formatted data frame so that each key has a column to denote the variable that it values
...
```{r}
# Pivot mod_1_mj_df 
long_mj_df <- left_j_mj %>% 
  pivot_longer(
    cols = c(PTS, TRB, AST, number_game),
    names_to = "Stats",
    values_to = "Values"
  )
```
...
```{r}
long_mj_task <- task_2_summary_df %>% 
  pivot_longer(cols = c(avg_pts, avg_trb, avg_ast),
               names_to = "Mean Stats", values_to = "Values")
```
...
# Find Max Total Points for each stage
