...
Returns rows with matching IDs in both dataframes
...
Rows move to the resulting table only when there is a match in both dataframes based on the shared key
...
keeps all the rows from the left df (names_df) and matches from right df (scores_df)
...
you can see that all the rows from the left df remain in the resulting table, regardless if there is a match in the right df. when there is a match, the values from the right are added to the left. if theres is not match, the column from the right df are filled with NA
...
in the full join, all rows from both dfs are included in the resulting table, regardless if there is a match
...
keep rows from name_df that have a match in scores_df but does not keep columns from scores_df, it is a filtering function
...
in a semi join, only the rows from the 1st df (left) that have a match in the second df (right) are included in the resulting table. Unlike other joins, the semi join does not include any columns from the second df
...
in an anti join, the resulting table includes only rows from the first df (left) that do not match in the second df (right). This join effectively filters out any rows in the first df that share a key with the second df
...
full_j_mj_df <- full_join(mj_df, div_df, by = c("Opp"="Abbreviation"))
```
...
The purpose of pivot_longer is to transform a wide dataframe at which the selected variables are the key and their specified elements are the values.
...
Keys are the selected columns and values are the selected column's elements

##### Pivot Wider

```{r}
## Use Pivot Wider on long_df
pivot_wider(data = long_df, 
             names_from = "Subject", 
             values_from = "Score")
```
...
It can transform a long formatted data frame so that each key has a column to denote the variable and its values
...
```{r}
wide_mj_stat_df <- lng_mj_stat_df %>% 
  pivot_wider(
    names_from = "STATS",
    values_from = "VALUES"
  )
View(wide_mj_stat_df)
```
...
```{r}
## Begin Work Here
mod_2_mj_df <- mod_1_mj_df %>% mutate(
  Stage = case_when(
...
# Find total pts for each division and stage
  
# Max total pts for stage
  
# Join data so that you max the pts according to stage
```
