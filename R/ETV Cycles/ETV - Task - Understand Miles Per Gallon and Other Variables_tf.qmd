...
\*\*This is called an "R chunk"

You only need to install the packages once. After installing the packages, you can load them using the `library()` function.

-   Once you buy the book, you don't need to buy it again

**Libraries** are used to load functions that are not part of the base R package. In this case, we will be using the `dplyr` and `ggplot2` libraries. The `dplyr` library is used for [data transformation]{.underline}, while the `ggplot2` library is used for [data visualization]{.underline}.
...
```{r}
str(mtcars) ## Provides variables and their types 
```
