...
-   Each observation is stored in it own row
...
```{r}
mtcars  %>% select(mpg) %>% ggplot(aes(x=mpg)) + geom_histogram()
```

*Note 1: pass mtcars data frame with pipe*

*Note 2: select column mpg before just to be more specific*
...
```{r}
p1 <- mtcars %>% select(mpg) %>% ggplot(aes(x=mpg)) + geom_density() + labs(title = "Density Plot of Miles Per Gallon", x="Miles Per Gallon")
